...
<div class="container mx-auto p-4 max-w-3xl">
    ...
    @php($optsForEdit = old('options', $question->options->map(function($o){ return ['text'=>$o->text,'subject_id'=>$o->subject_id]; })->values()->toArray()))
    <script type="application/json" id="init-options-edit">@json($optsForEdit)</script>
    ...
    <form action="{{ route('questions.update', $question) }}" method="POST" class="space-y-4"
          ...
          x-init="options = JSON.parse(document.getElementById('init-options-edit').textContent)">
        @csrf
        @method('PUT')

        <div>
            <label class="block font-semibold">Pertanyaan</label>
            <input type="text" name="text" class="w-full border rounded px-2 py-2" required value="{{ old('text', $question->text) }}">
